...
try {
	...
	$settings = array(
		...
		'repoPath'	=> 'chevereto',
		'videoId'	=> 'vDw1K-Spm_E',
		'logoUrl'	=> 'https://chevereto.com/app/themes/v3/img/chevereto-blue.svg',
		'timeout'	=> array(
			'redirection' => 10
		),
		'redirect'	=> TRUE,
	...
	if(isset($_REQUEST['action'])) {
		...
		switch($_REQUEST['action']) {
			case 'download':
				...
				$license_arr = explode(':', $license);
				if(count($license_arr) !== 2) {
					throw new Exception('Invalid license format', 100);
				}
				$zipball_url = 'https://chevereto.com/api/download/latest/?license=' . $license;
				$download = fetch_url($zipball_url, __DIR__);
				$json_decode = json_decode($download);
				if(json_last_error() == JSON_ERROR_NONE) {
					throw new Exception($download_json->error->message, $download_json->status_code);
				} else {
			...
			case 'extract':
                ...
                if ($zip->open($zip_file) === TRUE) {
					$zip->extractSubdirTo($temp_dir, $settings['repoPath']);
                    $zip->close();
                    @unlink($zip_file);
                } else {
...
<div id="terminal" class="stop">CHEVERETO WEB INSTALLER v1.1
https://chevereto.com
--
...
<noscript><span class="status status--error">ERROR</span> JavaScript is needed to use this installer.</noscript><form id="keylogin">Please enter your license key:
<input id="key" autofocus autocomplete="off" required></input>

<button type="submit" class="hidden">Enter key</button></form></div>
	...
	<script>
		$(function(){
			...
			var keylogin = "#keylogin";
			var key = "#key";
			
			var inputFocus = setInterval(function() {
				if($(key).val().trim() == "") {
					$(key).focus();
				}
			}, 1000);
			...
			$(document).on("keyup change", keylogin + " input", function() {
				var val = $(this).val();
				$(this).val(val.trim());
				$("button", keylogin)[($(this).val().trim() == "" ? "add" : "remove") + "Class"]("hidden");
			});
			...
			var INSTALL = {
				vars: {},
				download: function(licenseKey) {
					var _this = this;
					writeConsoleCommand("Downloading latest Chevereto release from chevereto.com");
					$.ajax({
						url: url,
						data: {action: "download", license: licenseKey},
			...
			$(document).on("submit", keylogin, function(e) {
				e.preventDefault();
				var licenseKey = $(key).val().trim();
				if(licenseKey == "" || /^\w+:\w+$/.test(licenseKey) == false) {
					return;
				}
				$(terminal).removeClass("stop");
				$(this).hide();
				clearInterval(inputFocus);
				INSTALL.download(licenseKey);
			});
		});
	</script>
</body>
